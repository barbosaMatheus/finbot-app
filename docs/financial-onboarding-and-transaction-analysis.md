...
| Date | Change | Reason |
| --- | --- | --- |
| 2026-09-05 | Gameplan step 7 — the eval harness: `npm run eval:gameplan` (`finbot-api/src/eval/`) builds the plan for six pinned users (Sam; a fixed-day week with no paycheck; tight cash the day after payday; a dated savings goal; "not sure"; shared accounts), fails on any drift in target ids, caps, amounts, free cash, the shelf or the plan-level reasons, then narrates the plan, a scripted period's grade and one heads-up reply through the configured provider and reports per call the source, the fallback reason and the numbers the raw model text carried that its input did not, ending in the fabricated-number rate; `--traffic` summarizes the provenance columns of real periods, grades and replies per user. The port now keeps the raw model text and its invented numbers on every `Narration`, including the ones the template replaced; the same pins run under jest | The Phase 1 exit criterion (a fabricated-number rate of zero) and the architecture note's evaluation harness; with the template provider the rate is trivially zero, and the same script gives the real figure against Ollama or Anthropic |
...
| 2026-09-05 | Gameplan step 6 — the anchor screen in `finbot` (`src/features/gameplan/`, routes `/gameplan/anchor` and `/gameplan/settings` outside the tabs so a push deep link can land on them): the last grade with positives first and "what got in the way?" only after a miss, the three targets with why lines, alternates collapsed under "Not these? Swap one" (one swap, then the control disappears), the heads-up line with the conditional amount box (pre-filled from the proposal, ballpark chips when the line carried no figure, skippable) and its reply, "Got it". Home shows the current plan's three targets; chat's header carries the plan as context; account links to the anchor settings (payday or a day, time of day). Push taps route to the anchor or chat by the notification's `url`. A line the port cannot turn into an adjustment is now kept as a `heads_up` reflection (API: reflection kind added) | The cadence note's anchor (§1, §5) and the gameplan note's amount box (§5a); walked on web against the dev stack with the template provider: open, swap, heads-up, done, got-it, settings |
| 2026-09-05 | Gameplan step 5 — the anchor endpoints under `/gameplan`: `GET /anchor` (period, plan targets and alternates with why lines, the previous final grade, live free cash, the re-engage flag, settings), `POST /anchor/got-it`, `POST /anchor/swap` (one per period), `POST /anchor/heads-up/parse` and `POST /anchor/heads-up` (the two steps of the amount box: proposal, then the confirmed amount through the engine's yield order with the diff narrated), `POST /anchor/reflection` (stored, embedded, attributed one-off or structural), `POST /anchor/awareness-done`, `GET`/`PUT /settings`. Registered in the contract with the engine's types in Zod; app client types regenerated | The cadence note's anchor (§1, §5) and the gameplan note's model boundary (§5, §5a, §6): the user acknowledges, swaps, informs and reflects, and never writes a target |
| 2026-09-05 | Gameplan step 4 — persistence and jobs. Migration 016: `gameplan_periods` (one live period per user, the engine's Shortlist as JSON), `gameplan_targets` (definition JSONB with the why line and its source), `plan_revisions`, `period_grades`, `user_reflections`, `gameplan_nudges`, `gameplan_accruals`, `gameplan_push_sends`, anchor settings on `user_info`, `recurring_streams.dominant_bucket`. Jobs: `REFRESH_USER_ANALYSIS` (a confirmed user's sync re-runs classify → reconcile → recur in-process, then payday detection and nudges — the orchestration used to skip finished users entirely), `BUILD_GAMEPLAN`, `GRADE_PERIOD`, `EVALUATE_NUDGES`, `RUN_GAMEPLAN_SCHEDULER` (hourly). Review confirmation opens the first period; a payday closes a period and opens the next; the scheduler covers the anchor day, the payday fallback, mid-period grades, reminders and any finished user without a period | The cadence note's events and data (§2, §6) and the gameplan note's §7, §8, §10.4, §10.7; every stored plan carries the engine's own definitions so a grade replays from what the user was shown |
| 2026-09-05 | Gameplan step 3 — the `LlmProvider` port in `finbot-api/src/llm/`: `explain(plan · grade · diff)` and `parseAdjustment(text, vocabulary)` over an `LlmClient` adapter (Ollama for local, Anthropic `@anthropic-ai/sdk` for hosted, a deterministic fake for tests); model output is schema-validated and every number in its words must exist in its structured input, else a plain template speaks and the reason is recorded; a proposed heads-up amount the text never stated is dropped so the box opens empty. `LLM_PROVIDER` (template by default) selects the host in compose and `.env` | The model boundary of the gameplan note (§5, §5a, decisions 5 and 13) and the architecture note's abstraction so Ollama serves development and a hosted provider serves production as a configuration change; the containment check is what keeps the Phase 1 fabricated-number rate at zero once plans have words |
| 2026-09-04 | Gameplan step 2 — the pure planner engine in `finbot-api/src/gameplan/` (types, expected-bills, free-cash, candidates, adjustment, grading): expected bills with windows, carry-over and long-cadence accrual; free cash with the cash check; six target types ranked by main goal under the diversity rules; the heads-up yield order with a computed diff; met / close / missed grading. No database, no model; fixtures under `tests/gameplan/`, the team walkthrough (Sam, $2,200 biweekly → shelf $1,472, free cash $448, transfer $112, eating-out cap $136) reproduced as a test. Classification `class-v2` splits `Food & Drink` into `Groceries` / `Eating Out` and `Transportation` into `Fuel` / `Transportation` by PFC detailed category | The gameplan note (locked 2026-09-04) §1–§7, §10; the essential floor is groceries + fuel + transport and caps go only on discretionary buckets, which the single `Food & Drink` bucket could not express |
